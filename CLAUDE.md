...
- **EIS current-range policy is SELECTABLE (`eis_range_mode`: `"static"`|`"auto"`), static by default.** `_preamble_eis` pins (`set_autoranging ba {cr} {cr}`, min==max) for `"static"` — because in-loop range switching corrupts the spectrum on FW 1.6.01 for *some* cells (low-freq Nyquist arc reverses / Z' goes negative, seen on 3-electrode). `"auto"` (GUI "Range selection" dropdown, or the `run_eis` tool enum) emits the original window `set_autoranging ba 1n {cr}` — needed for **high-impedance 2-electrode IDE cells**, where a pinned range buries the low-frequency points (current ≪ full scale → scattered/negative -Z'' tail). **In `"auto"`, `cr` is the CEILING (max range), not a starting point — the autoranger only steps DOWN from it toward the `1n` floor; in `"static"`, `cr` is the pinned range.** Options/default live in `scripts.EIS_RANGE_MODES` (single source for GUI+agent). Do NOT hard-pin EIS again: pinning vs autoranging depends on the electrode setup, so it stays a user choice. GEIS is still pin-only. Mode-2 techniques (CV/CA/...) keep real autoranging. Mode-3 has a different valid current-range ladder (`100n,1u,6u,13u,25u,50u,100u,200u,1m,5m`); the mode-2 values `2u`/`63u` return no data in mode 3 (see `parameter_form.current_ranges_for`)
