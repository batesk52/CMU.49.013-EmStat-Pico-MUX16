...
### 2026-06-09: CMU.17.034 Preset Sequencer — Batch 3 (GUI assembly) merged — FEATURE COMPLETE
- [x | code-team Batch 3 | 2026-06-09] Tournament (3 coders), winner coder 1, merged to `plan/cmu-17-034-preset-sequencer` (`deefb32`). Task 6: `src/gui/sequence_panel.py` SequencePanel(QWidget) — reorderable QListWidget of step blocks (preset+repeat+delay), add/remove/Run/Stop, "step i of N", save/load *.mux16seq, builds a SequenceRunner from injected PresetManager+engine+connection_provider. Task 8: main_window `_build_sequence_dock()` tabified beside Settings/Log; sequence-mode suppresses the export prompt + disables Start mid-sequence (re-enabled on finished/error). Task 9: `.gitignore` adds *.mux16/*.mux16seq/presets.json globs; `git rm --cached presets/presets.json` (kept on disk as migration source; built-ins remain in code). Task 10 carry-forwards: bounded `_advance` re-poll cap -> sequence_error; post-import selects first real preset; +2 runner tests (busy-engine refusal, late-finish-after-halt no phantom step). Suite 100 passed. Winner uniquely aborts the engine on Stop (coder 2 only halted the runner, leaving the in-flight step running) + cleanest connection_provider injection + deleteLater/null teardown preventing cross-run signal leaks. Remaining (non-blocking): no test exercises the re-poll-cap timeout path; could graft coder-2's faithful is_start_enabled() Start-state capture. CMU.17.034 feature complete on the plan branch (3 batches); not yet PR'd/merged to main.

### 2026-06-09: CMU.17.034 Preset Sequencer — Batch 2 (import + save-mode + runner) merged
- [x | code-team Batch 2 | 2026-06-09] Tournament (3 coders), winner coder 2, merged to `plan/cmu-17-034-preset-sequencer` (`63a1307`). Task 4: "Import preset file..." entry in TechniquePanel preset combobox (QFileDialog -> load_from_path -> repopulate -> persist last_preset_file, cancel-rollback). Task 5: GUI save-as-preset now persists electrode_config_mode + re_ce_channels with symmetric restore on preset select. Task 7: `src/engine/sequence_runner.py` SequenceRunner(QObject) chains engine.start_measurement on measurement_finished, halts on error, gates on isRunning(), sequence_mode flag + per-step auto-save dir helper. Suite 93 passed. Winner uniquely handled the real-engine thread race (engine emits measurement_finished from inside its still-running QThread; coder 2 re-polls via QTimer.singleShot(20) instead of advancing synchronously and falsely tripping the isRunning guard). Carry-forward to Batch 3: add a re-poll timeout cap; select a sensible preset post-import (not "(No Preset)"); graft coder-3's stronger queue-integrity tests + start-refuses-when-running test. Note: a concurrent code-team tournament (branches tourn/b1-coder*, worktree wt-echemcp-mgr) runs in this same repo on feature/e-cheMCP — kept isolated.
...
### 2026-06-08: EIS low-frequency timeout + noise-robustness (autoranging + e_dc bias) — bench-validated on S005 ferri/ferro
- [x | Session | 2026-06-08] Two independent EIS fixes, both hardware-validated on espico1601 (device EP2RP5OI), landed together in one combined PR.
  - **Low-frequency multi-channel EIS timeout.** Multi-channel EIS truncated after channel 1 when `freq_end` ≤ ~1 Hz: a single low-frequency point holds the serial line silent longer than the flat 120 s `MEASUREMENT_TIMEOUT`, so the read loop saw an empty read mid-sweep and the PR #7 fast-confirm (2 s) declared end-of-measurement before channel 2 ran. Per-point EIS time is ~22 s at 5 Hz and climbs past 120 s near 1 Hz (bench captures). Fix: `_eis_data_read_timeout(freq_end)` in `measurement_engine.py` scales the per-read timeout to the lowest swept frequency (flat 120 s ≥ 2 Hz; `350/freq_end` below, clamped [120, 900] s) — a 1 Hz point gets 350 s; ≥ 2 Hz byte-for-byte unchanged. Normal termination still instant via `+` END_MEAS. New `tests/engine/test_eis_read_timeout.py` (18 tests). Bench-validated (user: "working great").
  - **EIS noise robustness.** Bench EIS on high-impedance / ferri-ferro cells was very noisy below ~100 Hz with VLF rail-clamping and Z′ sign-flips. Two root causes in `scripts.py`: (1) `_preamble_eis` pinned the current range with `set_autoranging ba {cr} {cr}` (min==max DISABLES autoranging, manual §14.50) → low-frequency points read at <0.1% of the 100 µA range (|Z| spans ~1000:1 across a sweep). (2) `_gen_eis`/`_gen_geis` hardcoded the final `meas_loop_eis` argument to `0`, but that argument is the **DC potential** (manual §14.46) — so every sweep ran at 0 V regardless of the requested `e_dc`. Fixes: (a) real autoranging window `set_autoranging ba 1n {cr}` (floor matches PalmSens Pico EIS examples; FW 1.6 changelog "fixed meas_loop_eis resetting auto-ranging" makes the old <80 Hz corruption that originally motivated the lock obsolete; galvano preamble already used a real window); (b) pass `e_dc`/`i_dc` into the DC argument; mislabeled docstrings corrected. New tests in `tests/techniques/test_scripts.py`.
  - **Bench validation (S005 5 mM ferri/ferro, same-sample A/B `20260608_130629…very_noisy` → `20260608_133704`):** LF (<100 Hz) median point-to-point roughness dropped ~5–14× (|Z| 3.7–5.7% → 0.3–0.4%, Z′ 4.5–7.5% → 0.5–0.9%, Z″ 6.3% → 1.0%); HF unchanged; VLF rail-clamping gone; |Z| non-monotonic glitches 3–4/sweep → 1; spectrum shape shifted (max |Z| 279k → 115k) confirming `e_dc` now reaches the cell. 97 tests green.
  - Also includes a user preset edit (`TSC_EIS_2`: freq_end 10→1, channels →[1,2]).
  - Investigation method: multi-agent workflow (data noise-characterization + MethodSCRIPT v1.6 firmware research + code audit + adversarial syntax verification + completeness critic) — confirmed `eis_opt` per-point averaging is NOT available on the EmStat Pico (EmStat4-only, manual §9.8) and `set_max_bandwidth` is inert for EIS.
  - Next (EIS noise follow-ups, not yet done): #2 raise AC amplitude default (~25 mV, verify linearity); #4 equilibrate at `e_dc` before the `t_eq` wait; #5 software sweep-averaging (with a stationarity check — naive averaging of a drifting cell is counterproductive); #8 mains 50/60 Hz pickup (one residual Z′ sign-flip point remains in the 100–250 Hz band); optional Kramers-Kronig validity gate.
