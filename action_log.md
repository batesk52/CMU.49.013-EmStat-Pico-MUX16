...
### 2026-06-09: CMU.17.042 feature/e-cheMCP — embedded Claude agent, all 4 batches merged
- [x | code-team CMU.17.042 | 2026-06-09] Full embedded-agent stack built on `feature/e-cheMCP` via 3-coder tournaments per batch (manager-created worktrees `wt-b*-coder*` off explicit base SHAs; no harness worktree teardown incidents). B1 foundation (`src/agent/bridge.py` Qt<->asyncio marshaling, `mock_engine.py` signal-parity mock, `engine_adapter.py` config builders + race-free await-completion) — winner coder 1, +2 review fixes (emit-before-clearing-running; abort-on-await-timeout). B2 (`tools.py` 10 Anthropic tool defs + ToolRegistry + dispatch; `agent_worker.py` AsyncAnthropic streaming manual tool-use loop in a QThread asyncio loop, adaptive thinking gated to fable-5/opus-4-8/sonnet-4-6, lazy client, client_factory test seam) — winner coder 1, +fixes (ok:false -> is_error; closed-cycle CV + Pico-rejects-CP constraints in tool defs). B3 base: vendored CMU.49.011 `analysis/dataloaders/utils` at `e35ccfc` into `src/vendor/electrochem_analysis/` (read-only, imports rewritten, Agg forced) + bundled demo .pssession. B3 (`vendor_analysis.py` 7 analysis tools w/ PNG figure sink; `gui/agent_dock.py` chat panel + tool cards + figure strip; `main_window.py` +104-line additive wiring incl. EMSTAT_AGENT_MOCK plot-mirroring) — winner coder 1 (only one to fix the agent-run plot-prep gap), +4 review fixes (figure cap, dock width, sink payload copy, modal-free agent-run finish). B4 single-coder: `src/mcp_server/stdio_server.py` MCP stdio server reusing the same tool defs (mock by default, EMSTAT_MCP_PORT for hardware). Gates: 7 headless smokes (engine adapter, tools, agent w/ fake client, analysis, dock offscreen, main window offscreen, MCP subprocess) all exit 0 with no hardware and no API key; suite 77 passed throughout. Follow-up same day: `export_session` tool (engine.result -> PsSessionExporter -> absolute path; default ./agent_exports/, gitignored) closes the run -> analyze chain for both the dock agent and the MCP server; 8th gate `smoke_export_session.py` proves mock CV -> export -> vendored CVAnalyzer round-trip.
