...
- **main_window.py** - Main application window (Req 6)
  * ``MainWindow(QMainWindow)`` with dock-based layout: left dock (ConnectionPanel, TechniquePanel, ChannelPanel, MeasurementControlPanel), centre (a ``QTabWidget`` of browser-style live-plot tabs — one ``EISPlotContainer`` per run, each closeable via an X and carrying a hover tooltip; every run opens a fresh tab to the right and never overwrites an existing plot, so a sequence accumulates one labelled tab per step (``1·CV``, ``2·EIS``, …) and successive single runs accumulate likewise. A pristine "Live" scratch tab previews the selected technique and is consumed by the next run), bottom dock (QPlainTextEdit log console with attached logging handler)
  * Wires all Qt signals: engine ``data_point_ready`` is routed (``_route_data_point``) to the active plot tab, engine ``measurement_started`` opens/labels a new tab for the technique actually starting without disturbing prior plots (``_on_measurement_started`` → ``_begin_run_tab``), ``tabCloseRequested`` closes a tab (``_on_plot_tab_close``, refusing only the tab a run is actively recording into), engine ``measurement_finished``/``measurement_error``/``channel_changed`` to status bar and panel state updates, control panel signals to engine ``start_measurement``/``abort``/``halt``/``resume``
